...
Constraint.prototype.enforce = function() {
  ...
  this.p1.position.add(corr.clone().divideScalar(2));
  this.p2.position.add(corr.clone().negate().divideScalar(2));
  // ----------- STUDENT CODE END ------------
};
...
function handleImpactEvents(event) {
  ...
  if (event.key === "ArrowUp") direction = keyMap.ArrowUp.clone();
  else if (event.key === "ArrowDown") direction = keyMap.ArrowDown.clone();
  else if (event.key === "ArrowLeft") direction = keyMap.ArrowLeft.clone();
  else if (event.key === "ArrowRight") direction = keyMap.ArrowRight.clone();
  else return; // no key is pressed
  // determine the point being looked at
  let particle = cloth.getLookedAtParticle();
...
function handleClickEvents(event) {
  ...
  for (let i = 0; i < cloth.particles.length; i++) {
    let direction = cloth.particles[i].position.clone().sub(particle.position);
    let offset = direction.normalize().multiplyScalar(scale);
...
Cloth.prototype.applyRain = function(strength, rate) {
  ...
  for (let i = 0; i < rate; i++) {
    ...
    let index = location.x + location.y * this.w;
    //debugger;
    particles[index].position.add(impulse);
    if (index - this.w >= 0) {
      particles[index - this.w].position.add(impulse.clone().divideScalar(3));
    }
    if (index + this.w < particles.length) {
      particles[index + this.w].position.add(impulse.clone().divideScalar(3));
    }
    if (index % this.w !== 0 && index !== 0) {
      particles[index - 1].position.add(impulse.clone().divideScalar(3));
    }
    if (index % this.w !== this.w - 1 && index !== particles.length - 1) {
      particles[index + 1].position.add(impulse.clone().divideScalar(3));
    }
  }
